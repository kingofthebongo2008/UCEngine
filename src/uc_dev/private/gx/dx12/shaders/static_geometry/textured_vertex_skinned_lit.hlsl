...
float4 transform_world(float4 v_os, float4x4 world)
{
    //three muls for greater accuracy
    float4 result = mul( v_os, world );
    return result;
}
...
interpolants main(input i)
{
    ...
    float4     skinned_world    = transform_world( float4(skinned_position.xyz,1), m_world );
    
    r.uv          = i.uv;
    //skinned_position.y = -skinned_position.y;             22
    r.position    = project_vertex( float4(skinned_position.xyz, 1 ), m_world, m_view, m_perspective );
    r.position_ws = skinned_world;
