...
float4 main( interpolants r ) : SV_Target0
{
    ...
    float3 normal_vs                  = mul(float4(normalize(cross(dudx, dudy)), 0.0f), m_view).xyz;
    float3 sun_light_direction_vs     = mul(float4(normalize(float3(1.0, 0.5, 0.5)), 0.0f), m_view).xyz;
