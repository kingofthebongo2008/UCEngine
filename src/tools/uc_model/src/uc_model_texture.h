    ...
    namespace model
    {
        ...
        namespace mip_level_computation
        {
            ...
            template <> struct sample_image<static_cast<int32_t>(image_type::r16_g16_b16_a16_float)>
            {
                ...
                static void store(int32_t x, int32_t y, void* img, int32_t pitch, int32_t width, int32_t height, float4 v)
                {
                    auto    address = reinterpret_cast<char*> (sample_address_write(x, y, img, pitch, width, height));
                    auto    as_int  = _mm_cvtps_ph(v.m_data,0);
                    auto    y0      = _mm_packus_epi32(as_int, as_int); // Pack down to 16 bits
                    *address        = _mm_cvtsi128_si64(y0);
                }
            };
            ...
            template <> struct sample_image<static_cast<int32_t>(image_type::r16_g16_b16_a16_unorm)>
            {
                ...
                static void store(int32_t x, int32_t y, void* img, int32_t pitch, int32_t width, int32_t height, float4 v)
                {
                    ...
                    auto    as_int     = _mm_cvtps_epi32(as_float);
                    auto    y0         = _mm_packus_epi32(as_int, as_int);        // Pack down to 16 bits
                    *address            = _mm_cvtsi128_si64(y0);
                }
            };
            ...
            template <> struct sample_image<static_cast<int32_t>(image_type::r8_g8_b8_a8_unorm)>
            {
                ...
                static void store(int32_t x, int32_t y, void* img, int32_t pitch, int32_t width, int32_t height, float4 v)
                {
                    ...
                    auto    as_float   = _mm_mul_ps(v.m_data, _mm_set_ps1(255));
                    auto    as_int     = _mm_cvtps_epi32(as_float);
                    auto    y0         = _mm_packus_epi32(as_int, as_int);        // Pack down to 16 bits
                    auto    y1         = _mm_packus_epi16(y0, y0);                // Pack down to 8 bits
                    *address           = _mm_cvtsi128_si32(y1);
                }
            };
            ...
            void test()
            {
                ...
                static float4 r1 = sample_image<static_cast<int32_t>(image_type::r16_g16_b16_a16_float)>::load(0, 0, nullptr, 0, 0, 0);
                static float4 r2 = sample_image<static_cast<int32_t>(image_type::r16_g16_b16_a16_unorm)>::load(0, 0, nullptr, 0, 0, 0);
                static float4 r3 = sample_image<static_cast<int32_t>(image_type::r8_g8_b8_a8_unorm)>::load(0, 0, nullptr, 0, 0, 0);
                static float4 r3 = sample_image<static_cast<int32_t>(image_type::b8_g8_r8_a8_unorm)>::load(0, 0, nullptr, 0, 0, 0);
                static float4 r3 = sample_image<static_cast<int32_t>(image_type::b8_g8_r8_x8_unorm)>::load(0, 0, nullptr, 0, 0, 0);
            }
